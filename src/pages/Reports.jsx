...
const Reports = () => {
  const auth = useAuth();
  const userId = auth?.user?.id || auth?.user?.user?.id || null;

  // Estado para controlar qué tab se muestra
  const [activeTab, setActiveTab] = useState('custom'); // 'custom' o 'incidents'

  // Panel de reportes custom (Analista / Superadmin)
    ...
    <div className="flex flex-col space-y-8 min-h-screen">
      ...
      {activeTab === 'custom' && (
        <div className="space-y-6">
          <CustomReportsPanel userId={userId} />
        </div>
      )}

      {/* Tab: Reportes de Incidentes */}
      {activeTab === 'incidents' && (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Agregados por tipo, severidad, conductor y zona con exportes
                CSV/PDF
              </p>
            </div>
